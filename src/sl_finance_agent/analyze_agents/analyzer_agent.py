...
from ..tools_file_write_read import tool_custom_file_read, tool_custom_file_write, copy_file_to_folder
...
3. Thirdly: Analyze and summary the content follow the skill 'senior-financial-dupont-analyst'. Then generate report file with `tool_custom_file_write` in markdown format, and return the path of the report file.
4. Finally: You must use `copy_file_to_folder` to copy the report file into the destination directory:'/app/backend/shared-files'. **And return the report file name**.
...
- If only the step - 'Finally' failed, also STOP and Return the final results.
"""
...
def create_analyzer_agent(model_obj: ModelObj):
    ...
    * The DuPont ``SKILL.md`` file is loaded at import time, so the
    ...
    return create_deep_agent(
        ...
        skills=["/skills/"],
        backend=FS_BACKEND,
        tools=[get_current_time, tool_custom_file_read, tool_custom_file_write, copy_file_to_folder],
